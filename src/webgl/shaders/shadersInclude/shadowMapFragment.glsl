    ...
    shadow = ShadowCalculationWithPCSSSampling(v_positionFromLight);
        ...
        shadow = ShadowCalculationWithCSMPCF5Sampling(float(index0), v_positionFromLight[index0], v_depthMetricSM[index0]);
        ...
        if (index0<(SHADOWCSMNUM_CASCADES-1) && diffRatio<1.) {
            index0 += 1;
            float nextShadow = 0.;
            nextShadow = ShadowCalculationWithCSMPCF5Sampling(float(index0), v_positionFromLight[index0], v_depthMetricSM[index0]);
